...
- **Request:** board members concert about large % of traffice on gsearch, pull out monthly trends of paid sessions, also comparing with different channels
...
- **Request:** sessions, orders and CVR by months

- **Results:**

```sql
SELECT 
        YEAR(DATE(ws.created_at)) year,
        month(DATE(ws.created_at)) month,
        COUNT(DISTINCT ws.website_session_id) sessions,
        count(DISTINCT o.order_id) orders,
        ROUND(count(DISTINCT o.order_id)/COUNT(DISTINCT ws.website_session_id)*100,2) CVR
